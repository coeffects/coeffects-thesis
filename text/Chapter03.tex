...
\ident{Names} \mapsto \ident{Types}$. The expressions include \ident{?param} to read implicit
...
For simplicity, we assume that all implicit parameters have a type $\sigma$. In that setting, 
...
\figcaption{Coeffect rules for tracking context-usage in data-flow language}
...
\figcaption{Semantics showing how past values are accessed in a data-flow language}
...
(\eg~in the Perl language) or statically using a type system. Tainting can be viewed as a special
...
\emph{change} the context -- not just be evaluating certain code block in a different scope (\eg by 
