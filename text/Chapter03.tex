...
$\lambda$-calculus language to encode the model. More formally, this could be, for example,
a Cartesian closed category. In that model, we can reuse variables arbitrarily and so it is not
...
rule, which represents pointwise composition. The bounded reuse calculus rules look as follows:

\begin{equation*}
\hspace{-0.5em}
\tag{\emph{app}}
\begin{array}{l}
\sem{ \coctx{\Gamma_1, \Gamma_2}{\aclrd{\textbf{r}} \cons (\cclrd{t} \sqcup \aclrd{\textbf{s}})} \vdash e_1 \, e_2 : \tau_2} = 
  \lambda (\mathbf{v_1}, \mathbf{v_2}) \rightarrow \\[-0.25em]
  \qquad\kvd{let}~g = \sem{ \coctx{\Gamma_1}{\aclrd{\textbf{r}}} \vdash e_1 : \tau_1 \xrightarrow{\cclrd{t}} \tau_2 }~\mathbf{v_1}\\[-0.25em]
  \qquad\kvd{in}~g~(\sem{ \coctx{\Gamma_2}{\aclrd{\textbf{s}}} \vdash e_2 : \tau_1 }~\mathbf{v_2})
\end{array}
\end{equation*}
...
\qquad\qquad\qquad (\mathbf{v_1}, (x_1, \ldots, x_{\cclrd{t}}), (x_{\cclrd{t}+1}, \ldots, x_{\cclrd{t}+\cclrd{s}} ), \mathbf{v_2})
\end{array}
\end{equation*}
...
Bounded linear logic \cite{logic-bounded} restricts well-typed terms to polynomial-time algorithms.  
...
We really need just: $\ast$, $+$, $1$, $0$, $\leq$

composing flat and structural - meta vs. language semantics
