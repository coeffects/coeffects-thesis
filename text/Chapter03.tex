...
\begin{equation*}
\tyrule{prev}
  {\coctx{\Gamma}{n} \vdash e : \tau}
  {\coctx{\Gamma}{n+1} \vdash \kvd{prev}~e : \tau}
\end{equation*}
\begin{equation*}
\tyrule{sub}
...
\paragraph{Example.} 
In the following example, \ident{counter} generates positive integers 
and \ident{tick} flips between $0$ and $1$:
%
\begin{equation*}
\begin{array}{l}
(\,   \kvd{if}~~(\kvd{prev}~\ident{tick})=0~\kvd{then}~(\lambda x \rightarrow \kvd{prev}~x)\\
\,\,\, \kvd{else}~(\lambda x \rightarrow x) \,)\quad(\kvd{prev}~\ident{counter})
\end{array}
\end{equation*}
%
The function on the left-hand side returns the previous value of the argument every other 
step. Applied to a stream $\langle{\ldots,4,3,2,1}\rangle$ (where $1$ is the ``current''
value) it yields the stream $\langle{\ldots,4,4,2,2}\rangle$. To obtain the function, 
we need one past value from the context (for $\kvd{prev}~\ident{tick}$). The returned function 
needs either none or one past value (thus a subtyping rule is required to type it
as requiring one past value).
...
is $\langle{\ldots,5,4,3,2}\rangle$ and so the result is a stream $\langle{\ldots,5,5,3,3}\rangle$.

How many past values does the whole expression need? The argument requires one past value and
the function one additional ($\kvd{prev}~(\kvd{prev}~\ident{counter}))$, adding to $2$. To obtain
the function, one past value is needed. Overall we need the maximum of what is needed to evaluate the 
function and what is needed by passing the argument to the function: $max(1, 2)=2$. This is 
captured by (\emph{app}) in Figure~\ref{figure:intro-dataflow}. 

The (\emph{fun}) rule again does not allow delaying of context requirements. If the body 
requires $m$ past values, these have to be available both from the outer context and from the
argument provided by the caller. Later, we make the rule more flexible to make it compatible
with (\emph{fun}) for implicit parameters that \emph{splits} requirements.

\paragraph{Semantics.}
In this setting, a function $\tau_1 \rightarrow
