...
The syntax of the target programming langauge is shown in Figure~\ref{fig:semantics-target}. 
...
In Section~\ref{sec:semantics-proofs}, we will prove safety properties of well-typed programs in the 
...
\end{theorem}
\begin{proof}
  Rule induction over $\rightarrow^*$ using Theorem~\ref{thm:semantics-df-pres} and Theorem~\ref{thm:semantics-df-prog}.
\end{proof}


% --------------------------------------------------------------------------------------------------

\subsection{Coeffect langauge for implicit parameters}
\label{sec:semantics-proofs-impl}

As discussed earlier (Example~\ref{thm:semantics-indexed-prod}), implicit parameters can be modelled 
by an indexed product comonad, which annotates a value with additional context -- in our case, a 
mapping from implicit parameter names to their values. In this section, we embed this model into
the target language.
...
\paragraph{Liveness.}
For example, if we wanted to follow the development in the previous section for liveness, we would
extend the target language with two kinds of expressions, \ident{Dead} representing a dead context 
with no value and \ident{Live} representing a context with a value:
%
\begin{equation*}
\begin{array}{rcl}
...
\end{equation*}
%
The above example can be encoded using generalized algebraic data types (GADTs) in Haskell. We
define types \ident{D} and \ident{L} to capture the type annotations and we define a type
$\ident{C}~r~a$ where the type parameter $r$ represents a coeffect annotation:
%
\begin{equation*}
\begin{array}{l}
...
\end{equation*}
%
The operations of the flat coeffect algebra could then be encoded as type-level operations on
the types $\ident{L}$ and $\ident{D}$ so that, for example $\ident{Seq}~a~b$ is $\ident{D}$ only
if both $a$ and $b$ are $\ident{D}$. The types of the \ident{counit} and \ident{cobind} operations
would encode the coeffect annotations as:
...
\end{equation*}
%
Here, the additional type annotation ensures that \ident{counit} can only be called on a context
that contains a value and so calling the operation is not going to fail. Similarly, the type of the
\ident{cobind} operation now guarantees that if a function (used as the first argument) or the result
require a live context, it will be called with a value $\ident{C}~\ident{L}~a$ that is guaranteed
to contain a value. 

In our formalization, this is what guarantees that the reduction rules do 
not get stuck. For liveness, those would look as:
%
\begin{equation*}
\begin{array}{rl}
...
\end{equation*}
%
If we encoded the rules as functions in a dependently-typed language or in Haskell, the type
preservation property would be guaranteed by the type system of the target language. The 
equivalent of the progress property would then be guaranteed by the fact that the the operations
are always defined.
