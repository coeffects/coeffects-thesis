...
Compared to the previous chapter, structural coeffect calculi we consider are more 
...
Finally, we now also require two properties that relate the ordering $\cleq$ with coeffects composed
...
information are now tracked per-variable rather than for entire contexts. For data-flow,
...
of optional values (\ie~wrapped in the \ident{Maybe} data type). Additionally,
...
A semantics of structural coeffect calculi is shown in Figure~\ref{fig:struct-semantics}. As in
...
uses the $\ident{dup}_{\cclrd{s}, \cclrd{t}}$ operation to duplicate a variable while 
...
semantics fully determines the structure of the typing rules of the structural coeffect
calculus. 
...
In this section, we look at these common properties that we get ``for free'' from the structural 
