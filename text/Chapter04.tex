...
\item[--] We present a \emph{flat coeffect calculus} as a type system that is parameterized by a 
  ...
  instantiated to obtain three of the systems discussed in previous section (implicit parameters, 
...
$(\lambda x.e)\,v$ to a term $\subst{e}{x}{v}$, the soundness theorem is
stated as follows:

\begin{theorem}[Call-by-value soundness]
...
$\reduce_{\ident{cbn}}$ is the standard call-by-name reduction, the following soundness
theorem holds:

\begin{theorem}[Call-by-name soundness]
\label{thm:cbn-flat}
...
\begin{theorem}[Soundness of $\eta$-expansion]
In a bottom-pointed flat coeffect calculus with an algebra $(\C, \cseq, \cpar, \czip, \cunit, \czero, \cleq)$ 
