...
\subsection{Examples of indexed comonads}
...
%is provided by the indexed comonad $\ccat^rA = A \times (r \rightarrow \tau)$
...
%\begin{equation*}
%\begin{array}{l}
%\cobind{f}{r, s} \; : \; \ccat^{r \cup s} A \rightarrow \ccat^s B\\
%\cobind{f}{r, s} (a, g) = (f(a, g|_r), g|_s)
%\\[1em]
%\counit{} (a, g) = a\\
%%\iota_{r, s} \; : \: \ccat^r A \rightarrow \ccat^s A\\
%\iota_{r, s}(a, g) = (a, g|_s)\quad(\textnormal{if}~\leq r)
%\end{array}
%\quad\begin{array}{l}
%\cmerge_{r, s} : \ccat^r A \times \ccat^s B \rightarrow \ccat^{(r \cup s)}(A \times B)\\
%\cmerge_{r, s} ((a_1, g_1), (a_2, g_2)) = ((a_1, a_2), g_1|_{r \setminus s} \cup g_2 )
%\\[1em]
%\csplit_{r, s} : \ccat^{(r \cup s)}(A \times B) \rightarrow \ccat^r A \times \ccat^s B \\
...
% \begin{equation*} 
% \sem{\ctyp{r}{(x_1 : \tau_1 \times \ldots \times x_n : \tau_n)} \vdash e : \tau} : 
...
\sem{\ctyp{r}{\Gamma} \vdash \lambda x.e : \ctyp{s}{\tau_1} \rightarrow \tau_2} &=& 
    \emph{curry} \; (\sem{\ctyp{r \wedge s}{(\Gamma, x:\tau_1)} \vdash e : \tau_2} \circ \cmerge_{r, s})    %&$(1)$
\\[0.1em]
...
%    \sem{\ctyp{s}{\Gamma} \vdash e : \tau} \circ \iota_{r, s}                              %   &%$(4)$
%\\[0.1em]
%\sem{\ctyp{r\vee (r \cseq s)}{\Gamma} \vdash \kvd{let}~x=e_1~\kvd{in}~e_2 : \tau_2} &=&
%    \sem{\ctyp{r}{(\Gamma, x:\tau_1)} \vdash e_2 : \tau_2} \circ
% ((\varepsilon \times \sem{ \ctyp{s}{\Gamma} \vdash e_1 : \tau_1 }) \circ 
