...
idempotent monoid and $(\C, \czip)$ is a band (idempotent semigroup). That is, for all $r,s,t\in \C$: 
%
\begin{equation*}
\begin{array}{ccc}
...
If a written in a coeffect language with implicit parameters is well-typed according to the 
...
\begin{enumerate}
\raggedright
\item If $\coctx{\Gamma;\cclrd{\Delta}}{\cclrd{c}} \vdash x : \tau$ then $x : \tau \in \Gamma$ and $\cclrd{c} = \cunit$.
\item If $\coctx{\Gamma;\cclrd{\Delta}}{\cclrd{c}} \vdash n : \tau$ then $\tau = \ident{num}$ and $\cclrd{c} = \czero$.
...
\item If $\coctx{\Gamma;\cclrd{\Delta}}{\cclrd{c}} \vdash \lambda x\!:\!\tau_1.e : \tau$ then 
  $\tau = \tau_1 \xrightarrow{\cclrd{s}} \tau_2$ for some $\tau_2$ with 
...
Case (\emph{abs}): $e = \lambda x\!:\!\tau_1.e_1$ and $\cclrd{c}=\cclrd{\Delta}$.
  For some $\cclrd{r}$, 
  $\tau = \tau_1 \xrightarrow{\cclrd{r}\setminus\cclrd{\Delta}} \tau_2$ and also
  $\coctx{\Gamma, x\!:\!\tau_1;\cclrd{\Delta}}{\cclrd{r}} \vdash e : \tau_2}$. 
  ...
  ${\coctx{\Gamma, x\!:\!\tau_1;\cclrd{\Delta}}{\cclrd{r}} \vdash e : \tau_2'}$.
  By the induction hypothesis $\tau_2 = \tau_2'$ and so $\tau = \tau'$ and 
  also $\cclrd{c}=\cclrd{c'}=\cclrd{\Delta}$.
...
\paragraph{Ordering.}
Add $(\C, \cleq)$ is a pre-order that is:
