...
In the \clstr~calculus, this is not, in general, the case. The $\cprd$ operator is not required 
...
% ?? \cdupl_{r, s}  &:& T^{(r \cvop s)}A \rightarrow T^r A \times T^s A\\
%
...
The rule \ruless{app} now uses the operation $\csplit_{s, (r \cvop t)}$ instead of $\cdupl_{s, (r \cvop t)}$,
...
\item If we interpreted \rulessc{contr} by applying functor $T^{r\cvop s}$ to a function that duplicates
