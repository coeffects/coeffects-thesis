...
\ident{y}. In static binding, the context requirements of the body can be placed on the scope in
...
We replace a variable \ident{x} with an implicit parameter \ident{?x}. On the left, the
...
In the example on the left, \ident{?x} is bound to $10$ the function \ident{f} has no
delayed context requirements and the expression evaluates to $10$. In the example on the right,
the context requirement \ident{?x} is delayed and is satisfied via dynamic binding when calling
...
available. The type of the function \ident{f} on the left is $\kvd{int}\rightarrow\kvd{int}$, while
the type of the \ident{f} function on the right is $\{\ident{?x}:\kvd{int}\}\Rightarrow\kvd{int}\rightarrow\kvd{int}$.
...
In this thesis, we consider different notions of context, using implicit parameters as just one
