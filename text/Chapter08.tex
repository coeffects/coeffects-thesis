...
as some containers have multiple shapes of the same length, \eg~for perfectly ballanced, left- and right-leaning trees.}.
...
The coeffect container is a container that determines how are scalar coeffects attached to free-variable
...
can now be seen as a function $\SP_{\sclrd{l}} \rightarrow \C$ that returns a scalar coefect
...
such as Haskell that is inspired by the Haskell's ``do'' notation for monads
...
easiler embedding of context-aware computations themselves in programs akin to the ``do'' notation
for monadsin Haskell and one that allows tracking of the contextual information in the type system.
