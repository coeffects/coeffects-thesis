...
\emph{\cclrd{unified coeffect algebra}} (we repeat the definition below for clarity).
...
\emph{\aclrd{unified coeffect label}} which consists of shape-indexed coeffect algebra values.
...
that we now refer to it as \emph{\cclrd{unified coeffect algebra}}.

\begin{definition}
\label{def:unified-scal}
A \emph{\cclrd{unified coeffect algebra}} $(\C, \,\cseq, \,\cpar, \,\cunit, \,\czero, \,\cleq)$ is a set
...
The \emph{\aclrd{unified coeffect label}} combines a coeffect algebra and a coeffect container
...
Given a \cclrd{unified coeffect algebra} $(\C, \cseq, \cpar, \cunit, \czero, \cleq)$ and a
\sclrd{coeffect container} $(\SHP, \slen{-}, \stimes, \sempty, \sunit)$ a \aclrd{unified
coeffect label} extends the two structures with $(\azip, \apar, \azero)$ where $\azero \in \C^{\SP(\sempty)}$
